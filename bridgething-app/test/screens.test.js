...
test('usage still shows the contributing breakdown when the CLI reports no limit bars', () => {
  const html = renderUsage(baseState({
    usage: {
      updatedLabel: 'updated 01:46 · from claude /usage',
      limits: [],
      windows: [{
        window: 'Last 24h', requests: 305, sessions: 3, notes: [],
        skills: [{ name: '/android-pr-review', pct: '2%' }],
        subagents: [{ name: 'Explore', pct: '2%' }],
      }],
    },
  }));
  assert.doesNotMatch(html, /READING USAGE/);
  assert.match(html, /Last 24h · 305 requests · 3 sessions/);
  assert.match(html, /\/android-pr-review/);
});
...
// --- a long command must not push the card off its own edges -----------------
// The hero body is a column of fixed-height children — kind line, session name,
// chips — around one that wraps. At two stack rows the card is 236px and the
// budget only covers a single line of command, so a wrapped one used to push
// the header up under the hazard stripes and the chips' sublabels off the
// bottom. A command that wraps now buys its room from the stack, exactly as an
// intent line does.

const longCmd = 'npx --yes create-something --template typescript-strict --registry https://registry.internal.example.com/artifactory/api/npm/npm-virtual';

test('a command too long for one line buys the hero a row off the stack', () => {
  const three = (summary) => baseState({
    asks: [
      { kind: 'permission', id: 'p1', sessionName: 'proj', tool: 'Bash', summary, createdTs: Date.now() },
      { kind: 'permission', id: 'p2', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
      { kind: 'permission', id: 'p3', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
    ],
  });
  const rows = (html) => (html.match(/class="qrow[" ]/g) || []).length;

  assert.equal(rows(renderQueue(three('ls'))), 2,
    'a one-line command leaves the stack its full preview');
  assert.equal(rows(renderQueue(three(longCmd))), 1,
    'a wrapped command costs the stack a row, or the hero overflows its card');

  // The chips are the point of the screen: they and their sublabels survive
  // whatever the command does.
  const html = renderQueue(three(longCmd));
  assert.match(html, /ALLOW/);
  assert.match(html, /press dial/, 'the allow sublabel is still rendered');
  assert.match(html, /DENY/);
  assert.match(html, /preset 4/, 'the deny sublabel is still rendered');
});

test('the hero never eats the whole stack, however much it is asked for', () => {
  // Intent and a wrapped command each cost a row; together they must still
  // cost one, or a three-deep queue would show nothing under the hero.
  const html = renderQueue(baseState({
    asks: [
      { kind: 'permission', id: 'p1', sessionName: 'proj', tool: 'Bash', summary: longCmd, intent: 'you asked: reinstall everything', createdTs: Date.now() },
      { kind: 'permission', id: 'p2', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
      { kind: 'permission', id: 'p3', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
    ],
  }));
  assert.equal((html.match(/class="qrow[" ]/g) || []).length, 1);
  assert.match(html, /qhero has-intent/);
});

test('an unbroken token is measured like any other command', () => {
  // No spaces to wrap on: break-all is what fits it, and the length is still
  // what tells the layout it will take more than a line.
  const blob = 'https://example.com/' + 'a'.repeat(300);
  const html = renderQueue(baseState({
    asks: [
      { kind: 'permission', id: 'p1', sessionName: 'proj', tool: 'WebFetch', summary: blob, createdTs: Date.now() },
      { kind: 'permission', id: 'p2', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
      { kind: 'permission', id: 'p3', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
    ],
  }));
...
test('an empty command is still a one-line hero', () => {
  const html = renderQueue(baseState({
    asks: [
      { kind: 'permission', id: 'p1', sessionName: 'proj', tool: 'Bash', summary: '', createdTs: Date.now() },
      { kind: 'permission', id: 'p2', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
  ...
  assert.equal((html.match(/class="qrow[" ]/g) || []).length, 2);
});

test('the footer counts the cards actually on screen', () => {
  const asks = (summary) => [
    { kind: 'permission', id: 'p1', sessionName: 'proj', tool: 'Bash', summary, createdTs: Date.now() },
    { kind: 'permission', id: 'p2', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
    { kind: 'permission', id: 'p3', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
    { kind: 'permission', id: 'p4', sessionName: 'proj', tool: 'Bash', summary: 'ls', createdTs: Date.now() },
  ];
  assert.match(renderQueue(baseState({ asks: asks('ls') })), /4 waiting on you · showing 3/);
  assert.match(renderQueue(baseState({ asks: asks(longCmd) })), /4 waiting on you · showing 2/,
    'a hero that took a row back must not leave the footer promising it');
});
