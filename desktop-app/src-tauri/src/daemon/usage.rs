...
pub fn parse_usage(text: &str, now: i64) -> Option<Value> {
    ...
    // Some plans' /usage never shows a "NN% used" limit at all — just the
    // "what's contributing" windows/tables breakdown. That's still a real,
    // renderable reading; only truly empty output should count as a parse
    // failure.
    if limits.is_empty() && windows.is_empty() {
        return None;
    }
    let mut result = json!({
        "updatedTs": now,
...
mod tests {
    ...
    // Real output from CLI 2.1.227 (`claude -p "/usage"`, headless, no TTY
    // needed — confirmed identical under `script` too): no limit line at all,
    // just the subscription note and the Last 24h/7d contributing windows.
    // This used to return None because `limits` stayed empty, even though
    // the windows parsed fine and are worth showing on their own.
    #[test]
    fn parses_windows_only_output_with_no_limit_line() {
        let text = concat!(
            ...
            "\n",
            "Last 24h · 305 requests · 3 sessions\n",
            "  91% of your usage came from sessions active for 8+ hours\n",
            "  90% of your usage was at >150k context\n",
            "  Top skills: /android-pr-review 2%, /validate-tech-stack 1%\n",
            "  Top subagents: Explore 2%\n",
            "  Top MCP servers: claude.ai Atlassian 1%\n",
            "\n",
            "Last 7d · 1001 requests · 10 sessions\n",
            ...
            "  Top skills: /android-pr-review 3%, /jetbrains-debugger 1%, /build-and-install 1%, /create-pr 1%\n",
            "  Top subagents: Explore 7%, general-purpose 2%\n",
            "  Top MCP servers: notion 3%, android-studio-debugger 3%, claude.ai Atlassian 3%, android 2%\n",
        );
        let parsed = parse_usage(text, 0).expect("should parse windows even with no limit bars");
        assert_eq!(parsed["limits"].as_array().unwrap().len(), 0);
        assert_eq!(parsed["subscription"], "You are currently using your subscription to power your Claude Code usage");

        let windows = parsed["windows"].as_array().unwrap();
        assert_eq!(windows.len(), 2);
        assert_eq!(windows[0]["window"], "Last 24h");
        assert_eq!(windows[0]["requests"], 305);
        assert_eq!(windows[0]["sessions"], 3);
        assert_eq!(windows[0]["skills"][0]["name"], "/android-pr-review");
        assert_eq!(windows[0]["skills"][0]["pct"], "2%");
        assert_eq!(windows[1]["window"], "Last 7d");
        assert_eq!(windows[1]["mcp"].as_array().unwrap().len(), 4);
    }
}
